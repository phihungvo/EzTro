...
function AdminPaymentManagement() {
    ...
    const openAllocationModal = async (paymentId) => {
        ...
        try {
            ...
            message.error(getBillingUiErrorMessage(e, "Không tải được phân bổ"));
        } finally {
    ...
    useEffect(() => {
        ...
        getPaymentById(selectedPaymentId)
            .then((p) => {
                if (!cancelled) setSelectedPayment(p);
            })
            .catch(() => {
                if (!cancelled) setSelectedPayment(null);
            });
        return () => {
            cancelled = true;
        };
    ...
    const handleReceive = async (values) => {
        if (!selectedContractId) {
            message.warning("Hãy chọn hợp đồng trước.");
            return;
        }
        setSubmitting(true);
        try {
                ...
                content: (
                    <div style={{marginTop: 16}}>
                        <Descriptions bordered size="small" column={1} style={{marginBottom: 16}}>
                            ...
                            <Descriptions.Item
                                label="Bill liên quan">{payment.submittedBillCode || "—"}</Descriptions.Item>
                            <Descriptions.Item
                                label="Phương thức">{payment.paymentMethod ? formatEnumLabel(payment.paymentMethod) : "—"}</Descriptions.Item>
                            <Descriptions.Item
                                label="Người gửi / ghi nhận">{payment.createdByName || "—"}</Descriptions.Item>
                            <Descriptions.Item
                                label="Mã tham chiếu">{payment.externalReference || "—"}</Descriptions.Item>
                            <Descriptions.Item label="Chứng từ">
                                {payment.proofFileId ? (
                                    <Space>
                                        <span>{payment.proofFileName || `File #${payment.proofFileId}`}</span>
                                        <Button size="small"
                                                onClick={() => handleOpenProofFile(payment, "view")}>Xem</Button>
                                    </Space>
                                ) : "—"}
                            </Descriptions.Item>
                            <Descriptions.Item
                                label="Ghi chú từ tenant / thu ngân">{payment.note || "—"}</Descriptions.Item>
                        </Descriptions>
                        <Form form={confirmForm} layout="vertical">
                            ...
                            >
                                <TextArea rows={3}
                                          placeholder="VD: Đã đối chiếu sao kê BIDV lúc 08:15, khớp số tiền và nội dung chuyển khoản."/>
                            </Form.Item>
                            <Form.Item
    ...
    }, [confirmForm, describePaymentSource, handleOpenProofFile, selectedContractId, selectedPayment, refreshWorkspace]);
    ...
    const handleManualAllocate = async () => {
        if (!selectedPayment) {
            message.warning("Hãy chọn một khoản thanh toán.");
            return;
        }
        const allocations = outstandingInvoices
            .map((i) => ({
                billId: i.billId,
                amount: Number(i.manualAmount || 0),
                outstandingAmount: Number(i.outstandingAmount || 0)
            }))
            .filter((i) => i.amount > 0);
        if (!allocations.length) {
            message.warning("Nhập ít nhất một số tiền > 0 để phân bổ thủ công.");
            return;
        }
        if (allocations.find((i) => i.amount > i.outstandingAmount)) {
            message.error("Có khoản phân bổ vượt outstanding của bill.");
            return;
        }
        if (allocations.reduce((s, i) => s + i.amount, 0) > selectedPaymentUnallocated) {
            message.error("Tổng vượt số tiền chưa phân bổ.");
            return;
        }
        setSubmitting(true);
        try {
    ...
    return (
        <>
                                    ...
                                    <div className={cx("cardBody")}>
                                        ...
                                        <div className={cx("contractStrip")}>
                                            ...
                                            <div className={cx("contractStripItem")}>
                                                ...
                                                <span
                                                    className={cx("contractStripValue", "contractStripValue--accent")}>
                                                ...
                                                {(agingReport.buckets || [])
                                                    ...
                                                    .map((b) => (
                                                        <div key={b.bucketCode} className={cx("agingRow")}>
                                                            ...
                                                            <div>
                                                                <div
                                                                    className={cx("agingAmount")}>{formatCurrency(b.outstandingAmount)}</div>
                                                                <div className={cx("agingCount")}>{b.invoiceCount} hóa
                                                                    đơn
                                                                </div>
                                                            </div>
                                                        </div>
                                            ...
                                            <div className={cx("miniPanel", "miniPanel--red")}>
                                                ...
                                                {agingInvoices.slice(0, 4).map((inv) => (
                                                    <div key={inv.billId} className={cx("overdueRow")}>
                                                        <div>
                                                            <div
                                                                className={cx("overdueCode")}>{inv.billCode || `#${inv.billId}`}</div>
                                                            <div className={cx("overdueDays")}>Quá
                                                                hạn {inv.ageDays || 0} ngày
                                                            </div>
                                                        </div>
                                                        <span
                                                            className={cx("overdueAmount")}>{formatCurrency(inv.outstandingAmount)}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        )}
                                        ...
                                        {!selectedPayment ? (
                                            ...
                                            <>
                                                <Descriptions column={1} size="small" bordered>
                                                    {[
                                                        ["Mã thanh toán", <span
                                                            className={cx("detailMonoId")}>#{selectedPayment.id}</span>],
                                                        ["Mã tham chiếu", selectedPayment.externalReference || "—"],
                                                        ["Nguồn", describePaymentSource(selectedPayment)],
                                                        ["Trạng thái", <StatusTag status={selectedPayment.status}
                                                                                  meta={PAYMENT_STATUS_META}/>],
                                                        ...
                                                        ["Chứng từ", selectedPayment.proofFileId ? (
                                                            <Space>
                                                                ...
                                                                <Button size="small"
                                                                        onClick={() => handleOpenProofFile(selectedPayment, "view")}>Xem</Button>
                                                                <Button size="small"
                                                                        onClick={() => handleOpenProofFile(selectedPayment, "download")}>Tải</Button>
                                                            </Space>
                                                        ) : "—"],
                                                        ["Số tiền", <span
                                                            className={cx("detailAmtLg")}>{formatCurrency(selectedPayment.amount)}</span>],
                                                        ["Đã phân bổ", <span
                                                            className={cx("detailAlloc")}>{formatCurrency(selectedPayment.allocatedAmount)}</span>],
                                                        ["Chưa phân bổ", <span
                                                            className={cx("detailUnalloc")}>{formatCurrency(selectedPayment.unallocatedAmount)}</span>],
                                                    ...
                                                    {!(selectedPayment.allocations || []).length ? (
                                                        ...
                                                        (selectedPayment.allocations || []).map((a) => (
                                                            <div key={a.id} className={cx("allocHistoryItem")}>
                                                                <div>
                                                                    <span
                                                                        className={cx("allocBillId")}>Bill #{a.billId}</span>
                                                                    <span
                                                                        className={cx("allocTypeMeta")}>{formatEnumLabel(a.allocationType)}</span>
                                                                </div>
                                                                <span
                                                                    className={cx("allocAmt")}>{formatCurrency(a.amount)}</span>
                                                            </div>
                                                        ))
                                                    )}
                                                </div>
                                        ...
                                        {!creditEntries.length ? (
                                            ...
                                            creditEntries.map((e) => (
                                                <div key={e.id} className={cx("creditRow")}>
                                                    <div>
                                                        <div
                                                            className={cx("creditType")}>{formatEnumLabel(e.entryType)}</div>
                                                        {e.billId &&
                                                            <div className={cx("creditBill")}>Bill #{e.billId}</div>}
                                                        {e.createdAt && <div
                                                            className={cx("creditDate")}>{dayjs(e.createdAt).format("DD/MM/YYYY HH:mm")}</div>}
                                                    </div>
                                                    <span
                                                        className={cx(Number(e.amount || 0) >= 0 ? "creditPos" : "creditNeg")}>
                                        ...
                                        {!(billingAuditLogs || []).slice(0, 8).length ? (
                                            ...
                                            (billingAuditLogs || []).slice(0, 8).map((log) => (
                                                <div key={log.id} className={cx("auditItem")}>
                                                    <div
                                                        className={cx("auditOp")}>{formatEnumLabel(log.operationType)}</div>
                                                    ...
                                                    {summarizeAuditMetadata(log) && (
                                                        <div
                                                            className={cx("auditMeta")}>{summarizeAuditMetadata(log)}</div>
                                                    )}
                                                </div>
                                            ))
                                        )}
            ...
            <Modal
                title={`Phân bổ thanh toán #${allocationData?.paymentId || ""}`}
                open={allocationModalVisible}
                onCancel={() => setAllocationModalVisible(false)}
                footer={null}
                width={820}
            >
                <Spin spinning={allocationModalLoading}>
                    {allocationData ? (
                        <Space direction="vertical" style={{width: "100%"}} size="large">
                            <Row gutter={16}>
                                <Col span={8}>
                                    <Descriptions column={1} size="small" bordered>
                                        <Descriptions.Item
                                            label="Tổng">{formatCurrency(allocationData.totalAmount)}</Descriptions.Item>
                                    </Descriptions>
                                </Col>
                                <Col span={8}>
                                    <Descriptions column={1} size="small" bordered>
                                        <Descriptions.Item
                                            label="Đã phân bổ">{formatCurrency(allocationData.allocatedAmount)}</Descriptions.Item>
                                    </Descriptions>
                                </Col>
                                <Col span={8}>
                                    <Descriptions column={1} size="small" bordered>
                                        <Descriptions.Item
                                            label="Chưa phân bổ">{formatCurrency(allocationData.unallocatedAmount)}</Descriptions.Item>
                                    </Descriptions>
                                </Col>
                            </Row>

                            <Table
                                rowKey={(r, idx) => `${r.billId}-${idx}`}
                                size="small"
                                pagination={false}
                                columns={[
                                    ...
                                    {title: "Số tiền", dataIndex: "amount", render: (v) => formatCurrency(v)},
                                    {
                                        ...
                                        render: (v) => <Tag
                                            color={v === "ALLOCATE" ? "green" : "red"}>{formatEnumLabel(v)}</Tag>,
                                    },
                                ...
                                dataSource={allocationData.allocations || []}
                            />
                        </Space>
                    ) : (
                        <div style={{textAlign: "center", padding: 16}}>Không có dữ liệu</div>
                    )}
                </Spin>
            </Modal>
        </>
